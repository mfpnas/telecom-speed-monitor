...
from .formatters import format_currency, data_extenso, format_percentage, format_mbps, format_br_number
import os
import pandas as pd
...
def build_statistics(styles, stats: dict) -> list:
    ...
    weekend_data = []
    weekend_data.append(["Período", "Mediana Download (Mbps)", "% da Contratada", "Mediana Upload (Mbps)", "% da Contratada"])
    ...
    if stats['throttling']['detected']:
        ...
        throttle_text = """
        Não foi possível confirmar a prática de throttling com os dados disponíveis. A análise comparativa entre dias úteis e fins de semana apresentou uma variação pequena, e o teste estatístico não indicou diferença significativa. Recomenda-se a continuidade das medições para obter mais dados e reavaliar o comportamento em períodos de maior tráfego.
        """
...
def build_financial_loss(styles, stats: dict, plan_name: str) -> list:
    ...
    elements.append(Paragraph(f"•\t Valor mensal estimado: {format_currency(stats['valor_mensal'])}", styles['body']))
    ...
    loss_data = []
    loss_data.append(["Período", "% Entregue", "% Não Entregue", "Valor Mensal", "Valor Efetivo", "Perda Mensal"])
    ...
    for periodo, row in stats['pct_stats'].iterrows():
        ...
        loss_data.append([label, f"{dl_pct:.1f}%", f"{nao_entregue:.1f}%", format_currency(stats['valor_mensal']), format_currency(valor_efetivo), format_currency(perda)])
    ...
    elements.append(Paragraph(f"• Perda média mensal (calculada com base na mediana geral de download): {format_currency(perda_media)}", styles['body']))
    elements.append(Paragraph(f"• Perda total individual em {stats['meses']} meses: {format_currency(perda_total)}", styles['body']))
    elements.append(Paragraph(f"Este valor é passível de restituição em dobro (CDC, art. 42, parágrafo único), totalizando {format_currency(perda_total * 2)}", styles['body']))
    elements.append(Spacer(1, 0.3*cm))
    ...
    elements.append(Paragraph(f"• Perda média por cliente: {format_currency(perda_total)}", styles['body']))
    elements.append(Paragraph(f"•\t Danos materiais coletivos: {stats['num_clientes']} x {format_currency(perda_total)} = {format_currency(stats['num_clientes'] * perda_total)}", styles['body']))
    elements.append(Paragraph(f"•\t Danos morais coletivos (R$ 5.000/cliente): {stats['num_clientes']} x R$ 5.000,00 = {format_currency(stats['num_clientes'] * 5000)}", styles['body']))
    elements.append(Paragraph(f"• Total estimado da ação civil pública: {format_currency(stats['total_acao_coletiva'])}", styles['body']))
    elements.append(PageBreak())
    return elements
...
def build_recommendations(styles, stats: dict) -> list:
    ...
    elements.append(Paragraph("2. Ajuizamento de ação individual – Caso não haja solução administrativa, propôr ação perante o Juizado Especial Cível ou Vara Cível competente, pleiteando: (a) restituição em dobro dos valores pagos a maior, conforme art. 42 do CDC (total estimado de {}); (b) indenização por danos morais no valor de R$ 10.000,00, com base nos precedentes do STJ e TJSP; (c) obrigação de fazer para que a operadora passe a faturar com transparência, discriminando a velocidade média mensal entregue e os incidentes de interrupção, com desconto automático proporcional.".format(format_currency(stats['perda_total_individual'] * 2)), styles['body']))
    elements.append(Spacer(1, 0.2*cm))
...
def build_executive_summary(styles: Dict, stats: Dict, address: str) -> List:
    ...
    A mediana de upload foi de {overall_ul:.1f} Mbps ({(overall_ul/stats['plan_upload'])*100:.1f}% do contratado) e o ping mediano foi de {ping_med:.1f} ms.
    {throttling_desc}
    Com base na velocidade entregue, o prejuízo financeiro individual acumulado em {stats['meses']} meses é de {format_currency(perda_total_individual)}.
    Esta perda, quando considerada para uma possível ação coletiva, pode ultrapassar {format_currency(total_acao_coletiva)}.
    """
    elements.append(Paragraph(resumo_texto, styles['body']))
    elements.append(Spacer(1, 0.5*cm))

    elements.append(Paragraph(f"Localidade: {address}", styles['body']))
    elements.append(Paragraph(f"Data da emissão: {data_extenso(None)}", styles['body']))
    elements.append(Spacer(1, 1.5*cm))

    elements.append(Paragraph("_________________________________________", styles['body']))
    elements.append(Paragraph("Responsável Técnico", styles['body']))
    elements.append(Spacer(1, 0.5*cm))
    elements.append(Paragraph("_________________________________________", styles['body']))
    elements.append(Paragraph("Advogado", styles['body']))
    elements.append(Spacer(1, 1*cm))
