...
def compute_statistics(clean_df: pd.DataFrame, plan_download: float = 500,
    ...
    # Throttling - usando teste estatístico mais rigoroso
    throttling = {'detected': False, 'percent': 0}
    weekday_data = clean_df[~clean_df['IsWeekend']]['Download_Mbps']
    weekend_data = clean_df[clean_df['IsWeekend']]['Download_Mbps']
    
    if len(weekday_data) > 0 and len(weekend_data) > 0:
        from scipy import stats
        stat, p_value = stats.mannwhitneyu(weekday_data, weekend_data, alternative='two-sided')
        if p_value < 0.05:  # Diferença estatisticamente significativa
